...
const ContextoDeBarraDeProgreso =
...
function BarraDeProgreso({ active }: { active: boolean }) {
  return (
    <div
      aria-hidden={!active}
      className={[
...
export function ProveedorDeBarraDeProgreso({ children }: { children: ReactNode }) {
  ...
  return (
    <ContextoDeBarraDeProgreso.Provider
      value={{ startNavigation, isNavigating }}
    >
      <BarraDeProgreso active={isNavigating} />
      {children}
    </ContextoDeBarraDeProgreso.Provider>
  );
}

export function useBarraDeProgreso() {
  const context = useContext(ContextoDeBarraDeProgreso);
  if (!context) {
    throw new Error(
      "useBarraDeProgreso must be used within ProveedorDeBarraDeProgreso",
    );
  }
  return context;
}
...
export function useAppNavigation() {
  ...
  const { startNavigation } = useBarraDeProgreso();
